...
# (Settings, Subject Detection, Scraper, and other functions are the same)
...
def resolve_current_live_url():
    try:
        ...
        r = requests.get(live_page_url, timeout=15, headers=headers)
        r.raise_for_status()
        html = r.text
...
def upload_to_drive(local_path, subject):
    ...
    try:
        ...
        # 3. Get a direct, public link using 'rclone link'
        print("🔗 Generating direct public link...")
        result = subprocess.run(
            ["rclone", "link", remote_path_full],
            check=True,
            capture_output=True,
            text=True
        )
        ...
        if not direct_link:
            print("⚠️ Failed to generate direct link.")
            return None

        print(f"✅ Direct link generated.")
        return direct_link

    except subprocess.CalledProcessError as e:
        print(f"⚠️ An error occurred during rclone operation. Stderr: {e.stderr}")
        return None
    except Exception as e:
        print(f"⚠️ An unexpected error occurred during upload: {e}")
        return None
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            for ext in ['.mp4', '.mkv', '.webm']:
                if os.path.exists(base_path_without_ext + ext):
                    file_path = base_path_without_ext + ext
                    break
...
# (Main script is the same)
def main():
    print("🚀 Parmar SSC Auto Archiver v6 (Direct Link Method) started\n")
    ...
    if live_url:
        ...
        if info:
            ...
            if file_path and subject and video_info:
                gdrive_link = upload_to_drive(file_path, subject)
                if gdrive_link:
                    new_video_data = {"id": video_info.get("id"),"title": video_info.get("title"),"duration": time.strftime('%H:%M:%S', time.gmtime(video_info.get("duration", 0))),"uploadDate": datetime.now().strftime('%Y-%m-%d'),"startTime": "09:00","subject": subject,"gdrive_url": gdrive_link}
                    new_video_json = json.dumps(new_video_data)
        else: print("ℹ️ A video URL was found, but it is not currently live.")
    else: print("ℹ️ No stream is currently live.")
